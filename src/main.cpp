...
uint256 hashGenesisBlock("0x00000a15606617524911a3046ab21a09a88bd90b870fd1a49208b66768ba6036"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PioneerCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PioneerCoin Signed Message:\n";
...
{
    // PioneerCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 1)	        nSubsidy = 500000 * COIN; // first 50 blocks are 1 coin reward
else if(nHeight<200)		nSubsidy = 1 * COIN;
else if(nHeight<5000)		nSubsidy = 200 * COIN;
else if(nHeight<10000)		nSubsidy = 150 * COIN;
else if(nHeight<15000) 		nSubsidy = 100 * COIN;
else if(nHeight<50000)		nSubsidy =  50 * COIN;
else if(nHeight<1891600)	nSubsidy =  25 * COIN;

return nSubsidy + nFees;

}

static const int64 nTargetTimespan = 1 * 60 * 60; // PioneerCoin: 1 Hour
static const int64 nTargetSpacing = 1.0 * 60; // PioneerCoin: 1.0 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 576->60
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PioneerCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, PioneerCoin - DarkGravity v2, written by Evan Duffield - evan@PioneerCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, PioneerCoin - DarkGravity v3, written by Evan Duffield - evan@PioneerCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // PioneerCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PioneerCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x000009209a09988a1588517ef025322cb51b9ef2e9caf06a3ca90bafc3cb2823");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Zhao C.P.leaving from BlockChain to OKCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402508775;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 908150;

        if (fTestNet)
        {
            block.nTime    = 1402507543;
            block.nNonce   = 3861832406;
        }
...
*/
        ...
        assert(block.hashMerkleRoot == uint256("0x3dcfc3f65fcf14999963412a4bccba2302987e0c9b19e5502940d64690d0171f"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PioneerCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PioneerCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PioneerCoinMiner(CWallet *pwallet)
{
    printf("PioneerCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PioneerCoin-miner");
    ...
    try { loop {
        ...
        printf("Running PioneerCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PioneerCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PioneerCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - DARKCOIN **********
    Copyright 2014, PioneerCoin Developers 
        eduffield - evan@PioneerCoin.io
        InternetApe - kyle@PioneerCoin.io
*/
